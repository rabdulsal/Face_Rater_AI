...
class FRRatingViewModel : ObservableObject {
    ...
    private func processImageWithLabModel(_ image: UIImage) {
        ...
        let modelRequest = VNCoreMLRequest(model: try! VNCoreMLModel(for: labMod)) { [weak self] request, error in
            ...
            guard let self = self else { fatalError("Could not retrieve 'self' from callback \(#function)" )}
    ...
    private func processImageHotNotMod(_ image: UIImage) {
